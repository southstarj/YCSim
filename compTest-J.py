...
#Hwvalues = range(400);
#pvalues = range(299);
Jvalues = np.linspace(0.0001, 0.02, 1000);
alphaList = np.array([]);
comp_list = np.array([]);
dp_list = np.array([]);
pconv_list = np.array([]);
...
h1=plt.subplot(211)
plt.plot(Jvalues, alphaList, linewidth=2)
plt.plot(Jvalues, comp_list, '--', linewidth=2)
plt.tick_params(labelsize=20)
plt.xlim(0,0.02)
plt.ylim(-2000,2000)
plt.legend(['$\\alpha$', '$\\hat{\\alpha}$'],loc='lower right',fontsize=20)
h2=plt.subplot(212)
plt.plot(Jvalues, dp_list, '--')
plt.plot(Jvalues, pconv_list, '.')
plt.plot([0,0.02], [0,0], 'k')
#plt.xlabel('Injectivity $J$', fontsize=25)
plt.tick_params(labelsize=20)
plt.xlim(0,0.02)
plt.ylim(-2000,2000)
plt.legend(['$p^{k=1}$','$p_{converge}$'],loc='lower right',fontsize=20)
#plt.grid(True)
#plt.legend(['Nonlinear compressibility', 'Linear compressibility', 'Pressure after first iteration','Pressure after convergence'],loc='lower left',fontsize=20)
plt.show()
